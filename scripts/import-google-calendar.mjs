#!/usr/bin/env node
/**
 * Импорт событий Google Calendar → занятость в CRM (записи / пересечения со слотами).
 * Недельный график смены (`staff_schedule` в админке) — отдельный источник: слоты
 * строятся только внутри этих окон (см. work/src/lib/slots.ts). «Кто в какой день
 * работает» в общем календаре салона — это события; чтобы тянуть их в график, нужны
 * сопоставление имён с `staff` и отдельная логика (шаблон по дням недели или таблица
 * смен по датам), а не только этот импорт записей.
 */
