...
(function () {
  ...
  (function initServiceAndMasterPicks() {
    ...
    function syncMasterSelectEligibility() {
      ...
      /* При множественной корзине ничего не дисэйблим: master-select собран
       * из union (мастера, делающие хотя бы одну услугу), и любой из них
       * валиден к выбору — applyMasterToCoveredPicks разнесёт его per-service
       * (на услугах где умеет — ставит мастера, на остальных — «Не важно»).
       *
       * Раньше здесь работал жёсткий intersect-фильтр: при 3+ услугах с
       * разными командами мастеров allowed = пересечение часто было пустым
       * или неполным (например, расходилось с partial-флагом из-за гонки
       * setMasterOptions ↔ syncMasterSelectEligibility). В UI это выглядело
       * как «Aljona [disabled]» в глобальном select, хотя в карточке услуги
       * Aljona кликается без проблем. Клиент путался: «почему через карточку
       * можно, а через общий нельзя?». Теперь поведение единообразно:
       *   - в карточке pick — только её мастера (mastersForSpecificPick);
       *   - в общем select — union, без disabled, с пометкой «не на все
       *     услуги» у тех, кто не покрывает всю цепочку.
       * partial-суффикс остаётся как визуальный сигнал «не общий мастер». */
      if (picked.length > 1) {
        for (var u = 0; u < masterSelect.options.length; u++) {
          masterSelect.options[u].disabled = false;
        }
        return;
      }
      var allowed = mastersForPickedCategories();
     ...
     *  3) иначе если у услуги ЕСТЬ валидные мастера, но автовыбора нет —
     *     ставим ANY_MASTER_ID («Не важно»). Это убирает «мёртвую зону»
     *     старого поведения: при 3+ услугах без общего мастера новый pick
     *     получал selectedMaster="" → план дня показывал «мастер не выбран»,
     *     хотя у услуги есть мастера, готовые её выполнить. UX-цель: каждая
     *     строка корзины должна иметь явный статус (конкретный мастер или
     *     «Не важно»), а не молча зависать без значения;
     *  4) если же у услуги ВООБЩЕ нет валидных мастеров (CRM не привязал
     *     никого) — оставляем "" честно: этот случай UI должен подсветить
     *     отдельно, а не скрывать за «Не важно».
    ...
    function resolveDefaultMasterFor(pick, currentValue) {
      ...
      /* Несколько кандидатов и явного фаворита нет — ставим «Не важно»,
       * чтобы услуга не зависла без статуса. Это важный кейс: 3+ услуги
       * с разными командами мастеров (стрижка vs маникюр), где общий
       * глобальный выбор объективно невозможен. */
      if (allowed.length > 0) return ANY_MASTER_ID;
      return "";
    }
