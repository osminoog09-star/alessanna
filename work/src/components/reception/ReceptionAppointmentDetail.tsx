...
export function ReceptionAppointmentDetail({
  ...
  return (
    <div
      ...
      className="fixed z-50 overflow-hidden rounded-xl border border-zinc-700 bg-zinc-900 shadow-2xl"
      role="dialog"
    >
      {/* Color accent header */}
      <div
        ...
        style={{
          backgroundColor: colorStyle.backgroundColor as string,
          borderBottom: `1px solid ${colorStyle.borderColor as string}`,
        }}
      >
        <div className="flex items-start justify-between gap-2">
          <p className="text-sm font-semibold" style={{ color: colorStyle.color as string }}>
            {appt.client_name}
          </p>
          <button
            onClick={onClose}
            className="rounded-full p-0.5 opacity-70 hover:opacity-100"
            style={{ color: colorStyle.color as string }}
            aria-label="Закрыть"
          >
      ...
      <div className="space-y-2.5 p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium capitalize text-zinc-200">{dateLabel}</p>
            <p className="text-sm text-zinc-400">{timeLabel}</p>
          </div>
        </div>
        ...
        {svc && (
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-zinc-200">
              {svc.name_et}
              <span className="ml-1 text-zinc-500">· {svc.duration_min} мин</span>
            </span>
          </div>
        )}
        ...
        {appt.client_phone && (
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-zinc-200">{appt.client_phone}</span>
          </div>
        )}
      </div>
    </div>
